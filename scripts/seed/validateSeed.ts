...
export function validateSeedPayload(payload: SeedPayload): string[] {
  ...
  const expectedStats: Record<string, number> = {
    ...
    learningPathsCount: objectCount(learningPaths),
    practiceTasksCount: objectCount(practiceTasks),
    projectsCount: objectCount(projects),
    quizzesCount: objectCount(quizzes),
    announcementsCount: objectCount(announcements),
