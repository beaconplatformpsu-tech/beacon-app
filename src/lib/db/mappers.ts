...
export function mapUserProfile(uid: string, data: any): UserProfile {
  return {
    ...
    bio: data.bio || "",
    studyProgram: data.studyProgram || data.specialization || data.major || "",
    academicStage: data.academicStage || data.currentLevel || data.academicLevel || "",
    preferredCareerPathId: data.preferredCareerPathId || undefined,
    links: {
      github: data.links?.github || data.github || "",
      linkedin: data.links?.linkedin || data.linkedin || "",
    },
