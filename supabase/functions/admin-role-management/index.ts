// @ts-ignore - Deno runtime modules are resolved by Supabase at runtime.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // In our DB structure, the admin check is done via user_admin_meta or custom claims.
    ...
    // Note: Since we are using RTDB for authorization rules (e.g. root.child('user_admin_meta').child(auth.uid).child('role').val() === 'admin'), 
